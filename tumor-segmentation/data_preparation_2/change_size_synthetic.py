...
padded_img_folder = 'data/padded_synthetic_imgs'
padded_label_folder = 'data/padded_synthetic_masks'
...
def add_padding(img, target_size, padding_color):
    """
    Adds padding to the image to reach the target size.
...
for img_file in os.listdir(synthetic_images_folder):
    if img_file.endswith('.png') and img_file.startswith('synthetic_'):
        ...
        img_padded = add_padding(img, target_size, padding_color=(255, 255, 255))
...
for img_file in os.listdir(synthetic_mask_folder):
    if img_file.endswith('.png') and img_file.startswith('synthetic_'):
        ...
        img = Image.open(original_img_path)
        img_padded = add_padding(img, target_size, padding_color=(0, 0, 0))
        img_padded.save(padded_img_path)
